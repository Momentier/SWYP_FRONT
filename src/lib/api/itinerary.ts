 ...
 */
export const getItineraryDetails = async (
  id: number,
  ...
  try {
    ...
    const normalizedData = {
      ...
      dailyScheduleDtos: data.dailyScheduleDtos.map((dto) => ({
        ...dto,
        attractions: Array.isArray(dto.attractions)
          ? dto.attractions
          : [dto.attractions],
      })),
    };
...
const mergeItineraryByDate = (itinerary: ItineraryDetail): ItineraryDetail => {
  ...
  const mergedItinerary: DailyScheduleDtos[] = Array.from(
    itineraryMap.entries(),
  ).map(([dayDate, attractions]) => ({
    dayDate,
    attractions,
  }));
