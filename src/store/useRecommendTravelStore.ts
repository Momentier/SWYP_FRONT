...
//거리 및 시간 계산 로직
const calculateScheduleTimes = async (
  places: Attraction[],
): Promise<Attraction[]> => {
  if (!places) return [];
  ...
  return await Promise.all(
    places.map(async (currentPlace, index) => {
      ...
      if (
        ...
        try {
          const { walkingDuration, drivingDuration, distance } =
            await getRouteTime({
          ...
          if (walkingDuration)
            updatedPlace.travelWalkTime = `${walkingDuration}분`;
          if (drivingDuration)
            updatedPlace.travelCarTime = `${drivingDuration}분`;
...
export const useRecommendTravelDetailStore =
  create<RecommendTravelDetailStore>()(
    persist(
      (set, get) => ({
        itinerary: null,
        ...
        setItinerary: async (itinerary) => {
          if (itinerary?.dailyScheduleDtos) {
            const schedulesWithUUID = assignUUIDToSchedules(
              itinerary.dailyScheduleDtos,
            );
            const updatedSchedules =
              await updateDailySchedules(schedulesWithUUID);
            set({
              itinerary: { ...itinerary, dailyScheduleDtos: updatedSchedules },
            });
          } else {
            set({ itinerary });
          }
        ...
        updateItinerary: async (updatedItinerary) => {
          ...
          if (itinerary) {
            ...
            const updatedSchedules =
              await updateDailySchedules(updatedItinerary);
            set({
              itinerary: {
                ...itinerary,
...
const assignUUIDToAttraction = (attraction: Attraction): Attraction => {
  ...
  return {
    ...attraction,
    id:
      attraction.id && typeof attraction.id === "number"
        ? attraction.id
        : generateNumericId(), // 괄호 추가!
    previousData: attraction.previousData
...
const assignUUIDToSchedules = (
  schedules: DailyScheduleDtos[],
): DailyScheduleDtos[] => {
