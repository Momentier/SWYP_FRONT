...
const DetailCard: React.FC<DetailCardProps> = ({
  ...
  const updateAttraction = useRecommendTravelDetailStore(
    (state) => state.updateAttraction,
  );
    ...
    <div className="flex flex-col border-2 border-transparent rounded-2xl shadow-lg bg-[#F8F8F8] w-[880px] h-[208px] hover:border-[#9A77FF] transition-colors duration-200">
      <div className="flex p-6 gap-4">
        ...
        <div className="flex flex-col gap-2 w-[456px] h-[160px]">
          <Text
            textStyle="heading1"
            className="font-bold truncate overflow-hidden whitespace-nowrap"
          >
            {title}
          </Text>
          <Text
            textStyle="headline2"
            className="text-[#9A77FF] truncate overflow-hidden whitespace-nowrap"
          >
            {subtitle}
          </Text>
          ...
          <div className="flex items-center gap-2">
            <img
              src={COMMON_IMAGES.LOCATION}
              alt="address icon"
              className="w-5 h-5"
            />
            <Text textStyle="body1" className="text-gray-700">
              {address}
            </Text>
          </div>
          ...
          <div className="flex items-center gap-2">
            <img
              src={COMMON_IMAGES.CLOCK}
              alt="hours icon"
              className="w-5 h-5"
            />
            <Text textStyle="body1" className="text-gray-700 truncate">
              {hours}
            </Text>
          </div>
